  ...
  const Timer = {
    ...
    start: () => {
      ...
      interval = setInterval(() => {
        ...
        Timer.html.innerHTML = `${padLeft(Timer.time.minute)}:${padLeft(Timer.time.second)}.${padLeft(Timer.time.centi)}`
      }, 10)
      //Timer.time.start = Date().now
    },

    stop: () => {
      clearInterval(interval)
      //Timer.html.innerHTML = `<h1 class="stop">Stop</h1>`
      Timer.state = false
      localStorage[`Time_${localStorage.length + 1}`] = `${padLeft(Timer.time.minute)}:${padLeft(Timer.time.second)}.${padLeft(Timer.time.centi)}`
      saveTime()
      avg()
      random()
    },
